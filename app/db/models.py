...
from app.get_db().config import get_db    
...
class BaseModel(MappedAsDataclass, get_db().Model, IdMixin, TimestampMixin, SerializerMixin):
    __abstract__ = True
...
class ChatUser(MappedAsDataclass, get_db().Model):
