...
if not GOOGLE_CLIENT_ID:
    logging.error("Google CLIENT_ID not found in environment variables")
...
def generate_tokens(user: User):
    logging.info(f"Generating access and refresh tokens for user with email {user.email}")
...
def verify_token(token_data: TokenData, response: Response, db: Session = Depends(get_db)):
    ...
    logging.info("Verifying Google OAuth token and generating JWT")
    ...
    logging.info(f"Access and refresh tokens set in cookies for email {user_info['email']}")
...
def verify_google_oauth_token(access_token: str):
    try:
        ...
        if id_info.get("exp") < time.time():
            logging.error(f"Google OAuth token expired with expiration {id_info.get('exp')}")
        ...
        logging.info(f"Google OAuth token verified with user info {user_info}")
        return user_info

    except ValueError as e:
        logging.error(f"Google OAuth token verification failed with error: {str(e)}")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(e))
    

def validate_jwt_refresh_token(token: str):
    logging.info("Validating JWT refresh token")
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        if "sub" not in payload:
            logging.error(f"Token validation failed: 'sub' claim missing for token {token}")
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token")
        logging.info(f"JWT refresh token validated successfully for email {payload.get('email')}")
        return payload
    except JWTError as e:
        logging.error(f"Token validation failed with error: {str(e)}")
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token")
